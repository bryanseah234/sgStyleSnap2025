  ...
  return items.filter(item => {
    ...
    if (item.style_tags && Array.isArray(item.style_tags)) {
      ...
      if (recommendations.waterproof) {
        ...
        if (item.category === 'outerwear' && !item.style_tags.includes('waterproof') && !item.style_tags.includes('rain')) {
  ...
  try {
    ...
    for (const category of recommendations.categories) {
      ...
      if (categoryItems.length > 0) {
        ...
        outfit[category] = categoryItems[Math.floor(Math.random() * Math.min(3, categoryItems.length))];
      }
    }
...
export async function getWeatherAwareSuggestion(userId) {
  try {
    ...
    const suggestion = await generateWeatherAwareOutfit(
      userId,
      weather.temp,
      weather.condition
    );
