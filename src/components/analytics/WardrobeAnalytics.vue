<template>
  <div class="wardrobe-analytics">
    <!-- Header -->
    <div class="mb-6">
      <h2 class="text-2xl font-bold text-gray-900">Wardrobe Analytics</h2>
      <p class="text-gray-600 mt-1">Insights about your style and wardrobe usage</p>
    </div>
    ...
    <div v-else-if="analyticsStore.error" class="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
      <div class="flex items-start">
        ...
        <div>
          <h3 class="text-sm font-semibold text-red-800">Error loading analytics</h3>
          <p class="text-sm text-red-700 mt-1">{{ analyticsStore.error }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-else>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-lg shadow-sm p-6 text-white">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-blue-100 text-sm font-medium">Total Outfits</p>
              <p class="text-3xl font-bold mt-2">{{ analyticsStore.stats.total_outfits || 0 }}</p>
              <p class="text-blue-100 text-xs mt-1">Recorded in this period</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-yellow-500 to-yellow-600 rounded-lg shadow-sm p-6 text-white">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-yellow-100 text-sm font-medium">Avg Rating</p>
              <p class="text-3xl font-bold mt-2">
                {{ analyticsStore.stats.avg_rating ? analyticsStore.stats.avg_rating.toFixed(1) : '0.0' }}
              </p>
              <div class="flex items-center mt-1">
                <svg v-for="i in 5" :key="i" class="w-4 h-4" :class="i <= Math.round(analyticsStore.stats.avg_rating || 0) ? 'text-yellow-200' : 'text-yellow-400 opacity-30'" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-lg shadow-sm p-6 text-white">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-green-100 text-sm font-medium">Top Occasion</p>
              <p class="text-2xl font-bold mt-2 capitalize">
                {{ analyticsStore.stats.most_worn_occasion || 'N/A' }}
              </p>
              <p class="text-green-100 text-xs mt-1">Most frequently worn</p>
            </div>
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-sm p-6 text-white">
          <div class="flex items-center justify-between">
            <div>
              <p class="text-purple-100 text-sm font-medium">Items Used</p>
              <p class="text-3xl font-bold mt-2">{{ analyticsStore.stats.items_used || 0 }}</p>
              <p class="text-purple-100 text-xs mt-1">Different pieces worn</p>
            </div>
      ...
      <div v-if="analyticsStore.unwornCombinations.length > 0" class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div class="flex items-center justify-between mb-4">
          <div>
            <h3 class="text-lg font-semibold text-gray-900">Try Something New</h3>
            <p class="text-sm text-gray-600 mt-1">Unworn outfit combinations</p>
          </div>
          <button
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          >
            ...
            <p class="text-sm text-gray-600">{{ combo.items.length }} items • Never worn together</p>
          </div>
        </div>
      </div>

      <!-- Empty State -->
      <div v-if="analyticsStore.stats.total_outfits === 0" class="bg-white rounded-lg shadow-sm p-12 text-center">
        <svg class="w-20 h-20 mx-auto text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
        </svg>
        <h3 class="text-xl font-semibold text-gray-900 mb-2">No Analytics Data Yet</h3>
        <p class="text-gray-600 mb-6">Start recording your outfits to see insights about your style!</p>
        <button
