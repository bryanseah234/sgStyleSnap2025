...
export default function Cabinet() {
  ...
  const { data: outfits = [], isLoading: outfitsLoading } = useQuery({
    ...
    queryFn: () => {
      if (!currentUser?.email) return [];
      return api.entities.Outfit.filter(
        { created_by: currentUser.email },
        "-created_date"
      );
    },
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => api.entities.ClothingItem.delete(id),
    onSuccess: () => {
  ...
  const deleteOutfitMutation = useMutation({
    mutationFn: (id) => api.entities.Outfit.delete(id),
    onSuccess: () => {
  ...
  const toggleFavoriteMutation = useMutation({
    mutationFn: ({ id, is_favorite }) =>
      api.entities.ClothingItem.update(id, { is_favorite: !is_favorite }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["clothing-items"] });
    },
  });
  ...
  const toggleOutfitFavoriteMutation = useMutation({
    mutationFn: ({ id, is_favorite }) =>
      api.entities.Outfit.update(id, { is_favorite: !is_favorite }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["outfits"] });
    },
  });
