...
import SaveOutfitDialog from "../components/dashboard/saveoutfitdialog";
...
export default function Dashboard() {
  ...
  const { data: allItems = [] } = useQuery({
    ...
    queryFn: () => api.entities.ClothingItem.list("-created_date"),
  });

  const saveOutfitMutation = useMutation({
    mutationFn: (outfitData) => api.entities.Outfit.create(outfitData),
    onSuccess: () => {
  ...
  const loadOutfit = async (outfitId) => {
    try {
      const allOutfits = await api.entities.Outfit.list();
