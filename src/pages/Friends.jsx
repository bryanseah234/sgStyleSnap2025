...
export default function Friends() {
  ...
  const { data: incomingRequests = [] } = useQuery({
    ...
    queryFn: async () => {
      if (!currentUser?.email) return [];
      const allFriendships = await api.entities.Friendship.list("-created_date");
  ...
  const acceptRequestMutation = useMutation({
    mutationFn: async ({ requestId, requesterEmail, requesterName }) => {
      await api.entities.Friendship.update(requestId, { status: "accepted" });
  ...
  const declineRequestMutation = useMutation({
    mutationFn: (requestId) => api.entities.Friendship.update(requestId, { status: "declined" }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["friend-requests"] });
    },
  });
