...
const loadItems = async () => {
  try {
    if (currentUser.value?.email) {
      console.log('Cabinet: Loading items for user:', currentUser.value.email)
      const itemsData = await api.entities.ClothingItem.filter(
        { created_by: currentUser.value.email },
        '-created_date'
      )
      console.log('Cabinet: Loaded items:', itemsData)
      items.value = itemsData
    } else {
      console.log('Cabinet: No current user email found')
    }
  } catch (error) {
...
onMounted(async () => {
  try {
    ...
    console.log('Cabinet: Loaded user:', userData)
